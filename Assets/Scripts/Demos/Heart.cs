...
public class Heart : GenericBehaviour
{
    ...
    void SimpleHearth2()
    {
        ...
        for (int j = 0; j < 3; j++)
        {
            ...
            {
                float t = i + phase;
                float scale = (float)ScaleValue(PGApi.PGApi.ZgomotulPerlin_Îmbunătățit1D(t), 0, 1, 0.01f, scalePoints);
                ...
                float noise = ScaleValue(PGApi.PGApi.ZgomotulPerlin_Îmbunătățit2D(
                    Mathf.Cos(i) + 1 + j/5,
